...
export const allItemsPreviewState = selectorFamily({
    ...
    get: (count) => async () => {
        const { content } = await axiosInstance
            .get('/search/products?state=0&pageNum=0&size=8')
            .then((res) => res.data)
            .catch((err) => {
                console.error(err);
            });
...
export const imminentItemsPreviewState = selectorFamily({
    key: 'imminentItemsPreviewState',
    get: (count) => async () => {
        const { content } = await axiosInstance
            .get('/search/products?state=0&&sort=1&pageNum=0&size=8')
            .then((res) => res.data)
            .catch((err) => {
                console.error(err);
            });
